<template>
  <section id="bo_w" class="w-full mt-3 dark:bg-gray-700 dark:text-gray-400 dark:border-gray-400">
    <!-- 게시물 작성/수정 시작 { -->
    ...
    <form v-else name="fwrite" ref="fwrite" class="flex flex-wrap mb-4" id="fwrite" :action="v.action_url" v-on:submit.prevent="fwrite_submit" method="post" enctype="multipart/form-data" autocomplete="off" :style="{width : v.width}" >
      ...
      <ul class="flex w-full mb-3">
        <li v-if="v.is_notice" class="inline-flex items-center mt-3 ml-3"><input type="checkbox" id="notice" name="notice"  class="selec_chk form-checkbox h-5 w-5 text-blue-600" :value="'1'+v.notice_checked"><label for="notice" class="dark:bg-gray-700 dark:text-gray-400 ml-2 text-gray-700"><span></span>공지</label></li>
        <input v-if="v.is_dhtml_editor" type="hidden" value="html1" name="html">
        <li v-else class="inline-flex items-center mt-3 ml-3"><input type="checkbox" id="html" name="html" onclick="html_auto_br(this);" class="selec_chk form-checkbox h-5 w-5 text-blue-600" :value="v.$html_value + v.html_checked"><label class="dark:bg-gray-700 dark:text-gray-400 ml-2 text-gray-700" for="html"><span></span>html</label></li>
        <li v-if="v.is_admin || v.is_secret == 1" class="inline-flex items-center mt-3 ml-3"><input type="checkbox" id="secret" name="secret"  class="selec_chk form-checkbox h-5 w-5 text-blue-600" :value="'secret'+v.secret_checked"><label class="dark:bg-gray-700 dark:text-gray-400 ml-2 text-gray-700" for="secret"><span></span>비밀글</label></li>
        <input v-else type="hidden" name="secret" value="secret">
        <li v-if="v.is_mail" class="inline-flex items-center mt-3 ml-3"><input type="checkbox" id="mail" name="mail"  class="selec_chk form-checkbox h-5 w-5 text-blue-600" :value="'mail'+v.recv_email_checked"><label class="dark:bg-gray-700 dark:text-gray-400 ml-2 text-gray-700" for="mail"><span></span>답변메일받기</label></li>
      </ul>
      <div v-if="v.is_category" class="bo_w_select bo_w_info w-full px-3 mb-6 md:mb-0">
          <label for="ca_name" class="sound_only">분류<strong>필수</strong></label>
          <div class="relative">
          <select name="ca_name" v-html="'<option>분류를 선택하세요</option>'+ v.category_option" id="ca_name" class="dark:bg-gray-600 dark:text-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" required>
          </select>
      ...
      <div class="bo_w_info w-full md:w-1/2 px-3 mb-6 md:mb-0" v-if="v.is_name">
        <label for="wr_name" class="dark:text-gray-400 block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">이름</label>
        <input type="text" name="wr_name" :value="v.name" id="wr_name" required class="dark:bg-gray-600 dark:text-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" placeholder="이름">
      </div>

      <div class="bo_w_info w-full md:w-1/2 px-3 mb-6 md:mb-0" v-if="v.is_password">
        <label for="wr_password" class="dark:text-gray-400 block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">비밀번호</label>
        <input type="password" name="wr_password" id="wr_password" required class="dark:bg-gray-600 dark:text-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder="비밀번호">
      </div>
	
      <div class="bo_w_info w-full md:w-1/2 px-3 mb-6 md:mb-0" v-if="v.is_email">
        <label for="wr_email" class="dark:text-gray-400 block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">이메일</label>
        <input type="text" name="wr_email" :value="v.email" id="wr_email" class="dark:bg-gray-600 dark:text-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 email " placeholder="이메일">
      </div>

      <div class="bo_w_info w-full md:w-1/2 px-3 mb-6 md:mb-0" v-if="v.is_homepage">
        <label for="wr_homepage" class="dark:text-gray-400 block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">홈페이지</label>
        <input type="text" name="wr_homepage" :value="v.homepage" id="wr_homepage" class="dark:bg-gray-600 dark:text-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" size="50" placeholder="홈페이지">
      </div>
	
      <div class="write_div" v-if="v.option">
        <span class="sound_only">옵션</span>
        <ul class="bo_v_option dark:bg-gray-600 dark:text-gray-400" v-html="v.option"></ul>
      </div>

      <div class="bo_w_tit w-full px-3 mb-6 md:mb-4">
        <div class="flex justify-between">
          <label for="wr_subject" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2 dark:text-gray-400">제목</label>
        ...
        <input type="text" name="wr_subject" :value="v.subject" id="wr_subject" required class="dark:bg-gray-600 dark:text-gray-400 dark:border-gray-500 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" size="50" maxlength="255" placeholder="제목">
      </div>
