...
if ($board['bo_use_category']) {
    ...
    $category = array();
    $category[0]['name'] = '전체';
    $category[0]['href'] = str_replace(G5_URL, '',get_pretty_url($bo_table));
    $categories = explode('|', $board['bo_category_list']); // 구분자가 , 로 되어 있음
    for ($i=0; $i<count($categories); $i++) {        
        $category[$i+1]['name'] = trim($categories[$i]);
        if (trim($categories[$i]) =='') continue;
        $category[$i+1]['href'] = str_replace(G5_URL, '', get_pretty_url($bo_table,'','sca='.urlencode(trim($categories[$i]))));
        if ($category[$i+1]['name']==$sca) { // 현재 선택된 카테고리라면
            $category[$i+1]['active'] = true;
        }
    }
}
