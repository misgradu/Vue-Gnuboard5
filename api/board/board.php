...
add_event('alert', 'vue_alert',10,4); 
add_event('alert_close', 'vue_alert_close',10,2); 
add_event('alert_close', 'vue_confirm',10,4); 
        ...
        if (strstr($write['wr_option'], "secret"))
        {
            ...
            if (!$is_owner)
            {
                ...
                if (!get_session($ss_name)){
                    $redirect_url = str_replace(G5_URL, '', G5_BBS_URL.'/password?w=s&bo_table='.$bo_table.'&wr_id='.$wr_id.$qstr);
