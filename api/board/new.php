...
$gr_id = isset($_POST['gr_id']) ? substr(preg_replace('#[^a-z0-9_]#i', '', $_POST['gr_id']), 0, 10) : '';
if ($gr_id) {
...
$group_select[0]['label'] = '전체 그룹';
for ($i=0; $row=sql_fetch_array($result); $i++){
  $group_select[$i+1]['value'] = $row['gr_id'];
  $group_select[$i+1]['label'] = $row['gr_subject'];
}
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $list[$i]['href'] = str_replace(G5_URL, '', get_pretty_url($row['bo_table'], $row2['wr_id'], $comment_link));
