...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $list[$i]['num'] = $num;
    $list[$i]['opener_href'] = str_replace(G5_URL, '' , get_pretty_url($row['bo_table']));
    $list[$i]['opener_href_wr_id'] = str_replace(G5_URL, '' ,get_pretty_url($row['bo_table'], $row['wr_id']));
    $list[$i]['bo_subject'] = $row2['bo_subject'];
    $list[$i]['subject'] = $subject;
    $list[$i]['del_href'] = G5_BBS_URL.'/scrap_delete.php?ms_id='.$row['ms_id'].'&page='.$page;
}
$perPage = $config['cf_page_rows'];
