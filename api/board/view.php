...
if ($sca || $stx || $stx === '0') {
    ...
    $search_href = str_replace(G5_URL, '', get_pretty_url($bo_table,'','&page='.$page.$qstr));
    $list_href = str_replace(G5_URL, '', get_pretty_url($bo_table));
} else {
    $search_href = '';
    $list_href = str_replace(G5_URL, '', get_pretty_url($bo_table,'',$qstr));
}
...
if (isset($prev['wr_id']) && $prev['wr_id']) {
    ...
    $prev_href = str_replace(G5_URL, '', get_pretty_url($bo_table, $prev['wr_id'], $qstr));
...
if (isset($next['wr_id']) && $next['wr_id']) {
    ...
    $next_href = str_replace(G5_URL, '', get_pretty_url($bo_table, $next['wr_id'], $qstr));
...
if ($member['mb_level'] >= $board['bo_write_level']) {
    $write_href = str_replace('amp;', '', str_replace(G5_URL, '', short_url_clean(G5_BBS_URL.'/write.php?bo_table='.$bo_table)));
}

// 답변 링크
$reply_href = '';
if ($member['mb_level'] >= $board['bo_reply_level']) {
    $reply_href = str_replace('amp;', '', str_replace(G5_URL, '', short_url_clean(G5_BBS_URL.'/write.php?w=r&bo_table='.$bo_table.'&wr_id='.$wr_id.$qstr)));
}
...
if (($member['mb_id'] && ($member['mb_id'] === $write['mb_id'])) || $is_admin) {
    $update_href = str_replace('amp;', '', str_replace(G5_URL, '', short_url_clean(G5_BBS_URL.'/write.php?w=u&bo_table='.$bo_table.'&wr_id='.$wr_id.'&page='.$page.$qstr)));
    set_session('ss_delete_token', $token = uniqid(time()));
    $delete_href = G5_BBS_URL.'/delete.php?bo_table='.$bo_table.'&wr_id='.$wr_id.'&token='.$token.'&page='.$page.urldecode($qstr);
}
else if (!$write['mb_id']) { // 회원이 쓴 글이 아니라면
    $update_href = '/bbs/password?w=u&bo_table='.$bo_table.'&wr_id='.$wr_id.'&page='.$page.$qstr;
    $delete_href = '/bbs/password?w=d&bo_table='.$bo_table.'&wr_id='.$wr_id.'&page='.$page.$qstr;
}
...
if($v_img_count) {
    $view['File_Img'] .= "<div id=\"bo_v_img\">\n";
    for ($i=0; $i<=count($view['file']); $i++) {
        $view['File_Img'] .= get_file_thumbnail($view['file'][$i]);
    }
    $view['File_Img'] .= "</div>\n";
}
$is_signature = false;
...
if ($board['bo_use_signature'] && $view['mb_id']) {
    ...
    $view['signature'] = conv_content($signature, 1);
}
